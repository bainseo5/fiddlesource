...
// The base path where the pre-cut Doolin audio files are expected to be hosted.
// You must run `scripts/cut.py` and place its output files in a 'doolin_cuts' directory
// in your public assets (e.g., alongside index.html) for these URLs to work.
const CUT_AUDIO_BASE_PATH = 'doolin_cuts/';

// Global Session Metadata for Doolin
const DOOLIN_SESSION_LOCATION = "O’Connor’s Bar, Doolin";
...
const timeToSeconds = (timeStr: string): number => {
  ...
  } else if (parts.length === 2) { // Assuming MM:SS if only two parts
    return parts[0] * 60 + parts[1];
  }
  return 0; // Default to 0 if format is unexpected
};

// Helper to determine genre from title
...
// Replicates the title sanitization for filenames from cut.py
...
// Raw track data from the cut.py script, including original numbering and times
const original_track_data_raw = [
    ...
    // Fix: Changed parentheses to square brackets to correctly define array literals.
    ["52", "Reel_ Pigeon on the Gate (fragment)", "00:00:00", "00:00:20"], // Updated to be short test file, assuming a cut
...
const doolinTunes: Tune[] = [];
let currentOriginalNumTracking: string | null = null;
let subIndex: string = 'a';

original_track_data_raw.forEach((trackRaw, i) => {
  ...
  if (original_num_raw === currentOriginalNumTracking) {
      ...
      subIndex = 'a'; // Reset sub-index for a new original_num_raw
  }
  ...
  if (original_num_raw === "12" || original_num_raw === "44") { // Items 12 & 44: Micho Russell
      artist = "Micho Russell";
  } else if (original_num_raw === "40") { // Item 40: Paddy Killourhy, fiddle solo
      artist = "Paddy Killourhy";
  } else if (title.includes("(WC)")) { // Items marked (WC) include Willie Clancy
      artist = "Willie Clancy";
  } else if (title.includes("(JC)")) { // Items marked (JC) include Joe Cuneen
      artist = "Joe Cuneen";
  }

  // Sanitize title for display
  const cleanTitle = title.replace(/(\s\(WC\)|\s\(JC\)|\s\(all\))/g, '').trim();
  // Sanitize title for filename, matching cut.py logic
  ...
  doolinTunes.push({
    id: `doolin-${finalFilePrefix}`,
    title: cleanTitle,
    artist: artist,
    source: `${DOOLIN_SESSION_LOCATION} Session (Recorded by ${DOOLIN_SESSION_RECORDED_BY})`,
    region: DOOLIN_SESSION_LOCATION.split(',')[0].trim(), // Extract "Doolin"
    key: '?', // Key information not in source data
    tuning: 'Standard',
    year: DOOLIN_SESSION_YEAR,
    audioUrl: `${CUT_AUDIO_BASE_PATH}${audioFileName}`, // Point to the individual cut file
    ...
    startTime: 0, // Now that it's an individual file, start time is 0 within that file
    duration: duration,
    sourceCollection: 'O’Connor’s Bar Session, 1962',
    fileCutSource: true, // Flag this tune as coming from a pre-cut file
  });
});
