...
// Import tunes data from frontend (we'll load it dynamically)
// For now, we'll extract the tunes data and insert them
...
const seedDatabase = () => {
  try {
    ...
    // Check if tunes already exist
    const existing = db.prepare('SELECT COUNT(*) as count FROM tunes').get() as { count: number };
    if (existing.count > 0) {
      console.log(`✓ Database already contains ${existing.count} tunes. Skipping seeding.`);
      return;
    }
    ...
    const stmt = db.prepare(
      `INSERT INTO tunes (
        id, title, artist, source, region, key, tuning, year,
        audioUrl, description, genre, startTime, duration,
        sourceCollection, isImported, fileCutSource
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    );
    ...
    for (const trackRaw of original_track_data_raw) {
      ...
      stmt.run(
        `doolin-${finalFilePrefix}`,
        cleanTitle,
        artist,
        `O'Connor's Bar, Doolin Session (Recorded by ${DOOLIN_SESSION_RECORDED_BY})`,
        DOOLIN_SESSION_LOCATION,
        '?',
        'Standard',
        DOOLIN_SESSION_YEAR,
        `${CUT_AUDIO_BASE_PATH}${audioFileName}`,
        `Individual cut from the O'Connor's Bar session in Doolin, ${DOOLIN_SESSION_YEAR}. Features ${artist}.`,
        getGenre(cleanTitle),
        0,
        duration,
        "O'Connor's Bar Session, 1962",
        0,
        1
      );
    }
